...
#   filename:  materials_category/defective_structures/zero_dimensional/solid_solution/configuration.json
#   version:   0.28.5
...
class SupercellDimension(RootModel[conint(ge=1)]):
    root: conint(ge=1)
...
class SolidSolutionConfigurationSchema(BaseModel):
    ...
    """
    supercell_dimensions: Optional[List[SupercellDimension]] = Field(
        default_factory=lambda: [SupercellDimension.model_validate(v) for v in [1, 1, 1]], max_length=3, min_length=3
    )
    """
    Scaling factors [a, b, c] for the supercell.
    """
