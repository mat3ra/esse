...
class DatabaseSourceSchema29(BaseModel):
    id: Union[str, float]
    """
    ID string for the materials uploaded from a third party source inside the third party source. For materialsproject.org an example ID is mp-32
    """
...
class MaterialMetadataBoundaryConditions30(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
...
class CrystalSchema29(BaseModel):
    ...
    external: Optional[DatabaseSourceSchema29] = Field(None, title="database source schema")
    """
    information about a database source
    """
    src: Optional[FileSourceSchema] = Field(None, title="file source schema")
    ...
    metadata: Optional[
        Union[MaterialMetadataBoundaryConditions30, MaterialMetadataSlabProperties, MaterialMetadataBulkProperties]
    ] = {}
